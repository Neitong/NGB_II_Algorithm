...
g++ ../ex3.cpp -o student_program.out
...
output=$(LC_ALL=C ./student_program.out | tr -d '|-\n' | tr -s ' ' | xargs)
# output=$(./student_program.out | grep -v '[+|]' | xargs)

# Optional: Print the expected and actual output for debugging
echo "Expected Output:"
echo "$expected" | tee expected.txt
echo "Actual Output:" 
echo "$output" | tee actual.txt
